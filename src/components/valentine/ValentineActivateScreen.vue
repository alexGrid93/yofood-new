...
const focusTransform = computed(() => {
  ...
  const padX = 24 // отступы по бокам
...
function onOpenClick() {
  // тут показываешь калентинку (модалка/компонент/роут)
  console.log("OPEN VALENTINE")
}
...
<template>
  <div ref="rootEl" class="valentine-activate-screen" aria-hidden="true">
    ...
    >
      ...
      <button
        v-if="focusReady"
        class="focus-btn focus-btn--again"
        @click.stop="onChooseAgain"
      >
        Выбрать еще
      </button>
    </div>
